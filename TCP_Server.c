...
{ 
	...
	char buffer[7];
	printf("%ld",RandomNumber);
	//RandomNumber = ntohs(RandomNumber);
 ...
 while( 1 )
 {
	...
	else
	{
		...
		printf( "Received : %s\n", buffer );
         ...
         // ClientNumber = htons(ClientNumber);
         ClientNumber = atoi(buffer);
         
		 printf("random : %ld Client : %ld\n",RandomNumber,ClientNumber);
		 
		if(ClientNumber < RandomNumber)
		{
		  number_of_bytes_send = send( internet_socket, "Hoger\n", 6, 0 );
			  
          // char str[6];
          //sprintf(str, "%d", RandomNumber);
		  //send( internet_socket, str, sizeof(str)-1 , 0 );
		...
		if(ClientNumber > RandomNumber)
		{
			number_of_bytes_send = send( internet_socket, "Lager\n", 6, 0 );
	      if( number_of_bytes_send == -1 )
	       {
		    perror( "send" );
			break;
		...
		if(ClientNumber == RandomNumber)
		{
		   number_of_bytes_send = send( internet_socket, "You Win!\n", 9, 0 );
